...
public class UserProfileController {
	...
	public ResponseEntity<?> saveUser(@RequestBody() UserProfile user) {
		try {
			UserProfile addedUser = userService.registerUser(user);
			return new ResponseEntity<UserProfile>(addedUser,HttpStatus.CREATED);
	...
	public ResponseEntity<?> getUserList() {
			List<UserProfile> userList = userService.getAllUserProfiles();
	 ...
	 */
	@PutMapping("/{userId}")
	public ResponseEntity<?> updateUserDetails(@PathVariable("userId") String userId){
		try {
			UserProfile search = userService.getUserProfile(userId);
			UserProfile updatedUser = userService.updateUserProfile(search, userId);
	...
	public ResponseEntity<?> getUserDetails(@PathVariable("userId") String userId){
		try {
			UserProfile searchUser = userService.getUserProfile(userId);
			return new ResponseEntity<UserProfile>(searchUser,HttpStatus.OK);
		} catch (UserProfileNotExistsException e) {
			return new ResponseEntity<String>(e.getMessage(),HttpStatus.NOT_FOUND);
		}
	}
	...
	public ResponseEntity<?> deleteUserDetails(@PathVariable("userId") String userId){
		try {
